...
* **Python ≥ 3.10**
* A working **TELEMAC** installation (e.g. ``telemac2d``), reachable through its ``pysource.*.sh`` environment script - or, on Windows, its ``pysource.bat``. ``axqua`` does not import TELEMAC's Python; it *enters* that environment when the solver or SELAFIN tooling is needed (see :doc:`usage`).
...
recorded once rather than per case - describe them in ``~/.config/axqua/profiles.yml``
and check them with:
...
The **aXqua** QGIS plugin is a frontend for the same tool: it submits jobs, monitors
...
`GitHub release <https://github.com/sschwindt/aXqua/releases>`_.

.. important::

   **QGIS's Python does not need to be - and should not be - the solver's Python.** The
   plugin talks to the ``axqua`` command-line tool as a subprocess, so install
   axqua in the environment that reaches your solver and simply tell the plugin where
