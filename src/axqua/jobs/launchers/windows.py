...
``JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE`` must be **cleared**
    ...
    ``axqua submit`` exits, which is the opposite of detached.

The object must be **named**
    An unnamed Job Object is reachable only through the handle that created it, and that
    handle dies with the submitting process. A later ``axqua cancel``, in a different
    ...
    ``Local\\axqua-<job_id>`` plus ``OpenJobObjectW`` at cancel time.
...
log = logging.getLogger("axqua.jobs.launchers.windows")
    ...
    def _spawn(argv, cwd, env, creationflags, stdout):
        try:
            ...
            raise AxquaEnvironmentError(
                ...
                subject="launcher",
                remedy="Check that axqua is on PATH inside the solver environment.",
                argv=list(argv),
            ) from exc
