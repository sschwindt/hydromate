...
serialised into ``status.json``. A fresh ``axqua cancel`` days later reconstructs the
...
log = logging.getLogger("axqua.jobs.launcher")
...
def _registry() -> dict[str, type]:
    ...
    from axqua.jobs.launchers.windows import WindowsJobObjectLauncher
    from axqua.jobs.launchers.wsl import WslLauncher
...
def available_launchers(env: SolverEnvironment | None = None) -> list[str]:
    """Which launchers this machine can actually use. For ``axqua profiles``."""
...
def launcher_for(handle: LaunchHandle) -> JobLauncher:
    ...
    if cls is None:
        raise ConfigError(
            ...
            remedy="This job may have been launched by a different axqua version.",
        )
    return cls()
