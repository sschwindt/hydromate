...
``data_dir``
    Derived state axqua owns: ``jobs.sqlite``. Reconstructible (see
    :mod:`axqua.jobs.index`), so losing it costs a rescan and nothing else.
``state_dir`` / ``cache_dir``
...
ENV_CONFIG = "AXQUA_CONFIG_DIR"
ENV_DATA = "AXQUA_DATA_DIR"
ENV_STATE = "AXQUA_STATE_DIR"
ENV_CACHE = "AXQUA_CACHE_DIR"
ENV_JOB_ROOT = "AXQUA_JOB_ROOT"


#: The pre-rename application name. Its directories are still *read* when the aXqua ones
#: do not exist yet, so an existing job root, index and profiles.yml survive the rename
#: without anyone having to move anything.
LEGACY_APP_NAME = "hydromate"
_LEGACY_ENV_PREFIX = "HYDROMATE_"
_warned_legacy: set[str] = set()


def _env(name: str) -> Path | None:
    """Read *name*, falling back to its pre-rename spelling once, with a notice.

    A shell profile, a cron entry or a systemd unit written before the rename still
    exports ``HYDROMATE_JOB_ROOT``. Ignoring it would silently relocate someone's jobs,
    which is far worse than accepting it and saying so.
    """
    value = os.environ.get(name)
    if value:
        return Path(value).expanduser()
    legacy_name = _LEGACY_ENV_PREFIX + name.split("_", 1)[1]
    ...
    if legacy_name not in _warned_legacy:
        ...
        logging.getLogger("axqua").warning(
            "%s is the pre-rename name for %s; it still works, but rename it.",
            legacy_name, name)
    return Path(legacy).expanduser()


def _with_legacy(current: Path) -> Path:
    """Prefer *current*, but use the pre-rename sibling while only that one exists."""
    if current.exists() or LEGACY_APP_NAME == APP_NAME:
        return current
    legacy = current.parent / current.name.replace(APP_NAME, LEGACY_APP_NAME)
    return legacy if legacy != current and legacy.is_dir() else current
...
def config_dir() -> Path:
    ...
    if override:
        return override
    if os.name == "nt":
        return _with_legacy(_windows_base("APPDATA", "AppData/Roaming") / APP_NAME)
    if sys.platform == "darwin":
        return _with_legacy(Path.home() / "Library" / "Application Support" / APP_NAME)
    xdg = os.environ.get("XDG_CONFIG_HOME")
    return _with_legacy((Path(xdg) if xdg else Path.home() / ".config") / APP_NAME)
...
def data_dir() -> Path:
    ...
    if override:
        return override
    if os.name == "nt":
        return _with_legacy(_windows_base("LOCALAPPDATA", "AppData/Local") / APP_NAME)
    if sys.platform == "darwin":
        return _with_legacy(Path.home() / "Library" / "Application Support" / APP_NAME)
    xdg = os.environ.get("XDG_DATA_HOME")
    return _with_legacy((Path(xdg) if xdg else Path.home() / ".local" / "share") / APP_NAME)
...
class JobDir:
    ...
    def runner_log(self) -> Path:
        """axqua's narration. The solver's own listing is a separate artifact."""
