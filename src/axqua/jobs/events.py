...
* :class:`TeeSink` does both, which is what ``axqua execute`` wants - a debugging run
...
from axqua.jobs.model import JobStatus, Progress, utc_now

log = logging.getLogger("axqua.jobs.events")
...
class TextLogSink(NullSink):
    ...
    Deliberately separate from ``runner.log`` (plan §23): axqua's narration is state
...
current_sink: ContextVar[Any] = ContextVar("axqua_event_sink", default=NullSink())
