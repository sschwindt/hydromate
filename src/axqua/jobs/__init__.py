...
* :mod:`~axqua.jobs.model`, :mod:`~axqua.jobs.ids`, :mod:`~axqua.jobs.paths`
  are **standard library only**, so the CLI's job verbs stay as cheap as
  ``axqua status`` already is.
* :mod:`~axqua.jobs.executor` dispatches to a solver backend, which is exactly what
...
``axqua.jobs`` to read a job id must not drag in sqlite, yaml or a solver.
"""
...
if TYPE_CHECKING:  # pragma: no cover - for type checkers and IDEs only
    from axqua.jobs.events import (ConsoleSink, EventSink, NullSink, StatusFileSink,
                                       TeeSink, TextLogSink, current_sink, using_sink)
    from axqua.jobs.executor import ExecutionContext, JobCancelled, execute
    from axqua.jobs.ids import (JOB_ID_RE, is_job_id, make_job_id, parse_job_id, slug)
    from axqua.jobs.index import JobIndex, JobRow, list_jobs, scan
    from axqua.jobs.interaction import PolicyAsk
    from axqua.jobs.launcher import (JobLauncher, LaunchHandle, LaunchState,
                                         launcher_for, select_launcher)
    from axqua.jobs.lock import JobLock, LockedError, LockRecord
    from axqua.jobs.logs import RotatingTextSink, require_free_space, runner_log
    from axqua.jobs.model import (JOB_SCHEMA_VERSION, KIND_META, STATUS_SCHEMA_VERSION,
    ...
    from axqua.jobs.paths import (JobDir, config_dir, data_dir, index_path, job_root,
                                      profiles_path)
    from axqua.jobs.profiles import Profile, load_profiles, resolve, save_profiles
    from axqua.jobs.reaper import reconcile
    from axqua.jobs.results import (ResultEntry, ResultManifest, read_manifest,
                                        write_manifest)
    from axqua.jobs.store import (read_spec, read_status, write_json_atomic,
                                      write_spec, write_status)
    from axqua.jobs.submit import build_spec, submit_job
