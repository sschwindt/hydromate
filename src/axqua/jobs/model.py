...
class IllegalTransition(ConfigError):
    ...
    violation is a bug in axqua, and it should be loud.
    """
...
class Progress:
    ...
    a plugin built against an older axqua must degrade to showing nothing useful, not
    to losing the field.
    """
...
class SolverRunProgress(Progress):
    ...
    ``axqua.progress.SolverProgress`` already works this way.
    """
...
class KindMeta:
    ...
    ``verb`` is the :class:`~axqua.core.registry.SolverBackend` method the executor
    ...
    def from_dict(cls, data: dict[str, Any]) -> "JobSpec":
        ...
        if unknown:
            raise ConfigError(
                ...
                + ": " + ", ".join(unknown),
                subject=unknown[0],
                remedy="This job may have been written by a newer axqua. "
