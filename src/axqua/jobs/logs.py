...
``runner.log``
    axqua's own narration - state transitions, the commands it issued, exit codes,
...
#: 32 MiB x 5 for axqua's narration: generous for state transitions, bounded enough
...
log = logging.getLogger("axqua.jobs.logs")
...
def runner_log(path: str | os.PathLike, *, level: int = logging.DEBUG,
    ...
    """Route the ``axqua`` logger into a rotating ``runner.log`` for the duration.

    Scoped rather than global because a job runs inside a process that may also be doing
    something else (``axqua execute`` in a terminal), and the handler must come off
    cleanly afterwards.
    """
    target = Path(path)
...
class RotatingTextSink:
    ...
    def __init__(self, path: str | os.PathLike, *, max_bytes: int = SOLVER_MAX_BYTES,
        ...
        self._logger = logging.getLogger(f"axqua.solver-output.{id(self):x}")
...
def require_free_space(path: str | os.PathLike, minimum: int = MIN_FREE_BYTES) -> None:
    ...
    raise AxquaEnvironmentError(
        ...
        f"and axqua wants at least {minimum / 1024 ** 3:.1f} GiB",
        subject="resources.min_free_bytes",
        remedy="Free space, or point the job root at a larger volume "
               "(--job-root, $AXQUA_JOB_ROOT, or the profile's working_root).",
