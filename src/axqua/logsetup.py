...
handler) to the ``axqua`` logger and to the ``py.warnings`` logger, so a
...
``preprocessing/axqua.log``, the build (``pipeline.run``) -> the simulation
...
_TARGET_LOGGERS = ("axqua", "py.warnings")
...
def setup_logging(logfile: str | Path | None = None, *, level: int = logging.INFO,
    ...
    """Route axqua logs (+ warnings/errors) to a compound logfile and console.
    ...
    if logfile is not None:
        ...
        if str(path) not in _logfiles:
            ...
            log.info("=== axqua logging started -> %s "
