...
:mod:`axqua.core`, shared with every other backend, which is what lets a second
...
:mod:`~axqua.solvers.telemac.spec` is read by ``axqua status`` to build a
...
def __getattr__(name: str):
    if name in _SUBMODULES:
        return importlib.import_module(f"axqua.solvers.telemac.{name}")
    raise AttributeError(f"module 'axqua.solvers.telemac' has no attribute {name!r}")
