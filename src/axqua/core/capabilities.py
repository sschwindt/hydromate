...
``implemented``
    Does axqua support this capability **for this solver at all**? A static
    ...
    one is a gap in axqua, the other is a category error.
...
class CapabilityState:
    ...
    def as_dict(self) -> dict:
        """JSON form, for ``axqua case-status --json``.
        ...
        ``no`` means show it disabled with a reason (axqua has not built it yet),
...
class SolverStatus:
    ...
    def render(self) -> str:
        ...
        lines = [
            ...
            f"# generated by axqua {self.version} - regenerated by every build "
            "and by `axqua status`; do not edit",
            _HEADER_RULE,
            f"solver      = {self.name}",
...
class CaseStatus:
    ...
    def as_dict(self) -> dict:
        ...
        adding a capability - or a whole solver - to axqua surfaces in the plugin
...
def refresh_markers(cfg) -> list[Path]:
    ...
    rather than as it was when someone last ran ``axqua status``. A build that
    ...
    try:
        ...
        logging.getLogger("axqua").debug("could not refresh MODEL= markers",
                                             exc_info=True)
        return []
