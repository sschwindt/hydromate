...
``user-sources/geodata/``, see :mod:`axqua.targets`).
...
from axqua.targets import _norm_header
...
def fill_template_hydraulics(template: Path, sources, *, sheet_name: str = "hydraulics",
    ...
    if not template.exists():
        raise FileNotFoundError(
            f"template not found: {template}. Run `axqua targets <config>` first.")
...
# axqua.targets.write_target_template) so users get a co-located script.
...
Run in the axqua env, from this folder:

    mamba run -n axqua-env python extract_flowtracker.py FILE1.xlsx [FILE2.xlsx ...]
