"""Streamlit configuration editor for axqua.

A browser form to create / edit a axqua YAML config, mirroring the dataclass
schema in :mod:`axqua.config`, with the ability to save the YAML and run
``axqua --check`` (validate) or a full build directly from the browser.
...
Launch with ``axqua-gui`` (after ``pip install -e ".[gui]"``).
"""
...
def run_cli(args: list[str]) -> tuple[int, str]:
    """Run the axqua CLI and capture combined output."""
    proc = subprocess.run(["axqua", *args], text=True, capture_output=True)
...
#
# The production rating pipeline (axqua.rating) uses a *symmetric* section; the
...
st.set_page_config(page_title="axqua config editor", layout="wide")
st.title("axqua - configuration editor")
...
with st.sidebar:
    ...
    if st.button("Reset to template"):
        ...
        st.rerun()
    st.divider()
    st.caption("Launch: `axqua-gui` (needs `pip install -e \".[gui]\"`). "
...
with tabs[0]:
    ...
    p["preprocessing_dir"] = c1.text_input(
        "Preprocessing dir",
        _get(cfg, "project", "preprocessing_dir", default="axqua-case/preprocessing"))
    p["model_dir"] = c2.text_input(
        "Simulation (model) dir",
        _get(cfg, "project", "model_dir", default="axqua-case/simulation"))
    p["postprocessing_dir"] = c1.text_input(
        "Postprocessing dir",
        _get(cfg, "project", "postprocessing_dir", default="axqua-case/postprocessing"))
    p["calibration_dir"] = c2.text_input(
        "Calibration-validation dir",
        _get(cfg, "project", "calibration_dir", default="axqua-case/calibration-validation"))
...
with tabs[10]:
    ...
    st.caption("Standalone tool - it does **not** edit the config. Same normal-flow "
               "physics as `axqua rating` / `synthesize_outflow_rating`, extended "
...
`cases/<your-case>/` (`mamba run -n axqua-env python cases/<case>/<script>.py`):
...
1. **`preprocessing.py`** - build the full TELEMAC-2D case into `simulation/`
   ...
   *Equivalent to the **Build case** button here (`axqua <config>`).*
...
with right:
    ...
    if st.button("🚀 Build case"):
        if not Path(save_path).exists():
            ...
            with st.spinner("Running axqua… (meshing can take a while)"):
