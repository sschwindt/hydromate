...
def synthesize_rating_if_missing(cfg: Config, discharge: float,
    ...
    if method == "section":
        from axqua.rating import synthesize_outflow_rating_from_section
...
def format_flux_convergence(fc) -> list[str]:
    """Render a :class:`~axqua.flux_convergence.FluxConvergence` result as
...
def format_3d_cases(setups: dict) -> list[str]:
    """Render :func:`axqua.threed.build_3d_cases` results as user-facing report
    ...
    live in :mod:`axqua.threed` / :mod:`axqua.unsteady`.
    """
    purposes = {
...
def expected_duration(cfg: Config) -> float:
    ...
    Mirrors :mod:`axqua.steering`'s ``DURATION`` logic: the explicit
...
def mesh_from_geometry(cfg: Config):
    """Rebuild a :class:`~axqua.mesh.Mesh` from the built ``geometry.slf``.
...
def report_wetting(cfg: Config) -> list[str]:
    ...
    """
    from axqua.wetting import outlet_profile, wetting_report
...
def report_sections(cfg: Config, out_name: str = "baffle-XS-q.csv") -> list[str]:
    ...
    if cfg.geodata.control_sections is None:
        return []
    from axqua.sections import write_line_discharges
