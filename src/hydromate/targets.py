...
* :func:`read_target_parameters` turns the ``parameters`` tab into
  :class:`~hydromate.config.CalibrationParameter` rows with HydroBayesCal names
...
zip-level reader from :mod:`hydromate.ground_truth`.
"""
...
from hydromate.config import CalibrationParameter, Config
...
def write_target_template(cfg: Config, path: Path | None = None,
    ...
    ws.title = "README"
    readme = [
        f"Calibration target data for case '{cfg.name}' (generated by hydromate)",
        "",
        ...
        "overwrite TKE with a measured value if you have one. hydromate recomputes",
        ...
        "re-run preprocessing.py (or the hydromate build) to regenerate the",
    ...
    def _sheet(name: str, headers: tuple[str, ...], comments: dict[str, str],
        ...
        for j, h in enumerate(headers, start=1):
            ...
            if h in comments:
                cell.comment = Comment(comments[h], "hydromate", height=120, width=320)
    ...
    from hydromate.flowtracker import write_driver_script
