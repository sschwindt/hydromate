"""What hydromate raises, and why the type matters more than the message.
...
class HydromateError(Exception):
    """Base class: a failure hydromate can describe rather than merely report.
...
class ConfigError(HydromateError):
    ...
    code = "hydromate.config"


class GeodataError(HydromateError):
    """An input layer or raster cannot be used: absent, wrong CRS, missing column."""

    code = "hydromate.geodata"


class EnvironmentError(HydromateError):  # noqa: A001 - shadows the builtin knowingly
    ...
    that wants it (it is an alias of ``OSError``, which hydromate never raises
    deliberately).
    """
...
class SolverError(HydromateError):
    """The solver ran and failed. ``listing`` points at what to read."""

    code = "hydromate.solver"


class MeshError(HydromateError):
...
class ErrorRecord:
    ...
    generic code, which is honest: it says "this was not a failure hydromate
