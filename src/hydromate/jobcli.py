...
because a QGIS plugin's entire view of hydromate is this file's output. Two properties
...
``{"ok", "command", "hydromate", "data", "error"}``. The document goes to **stdout** and
...
out what went wrong. The ``HydromateError`` categories already carry stable codes, so they
map onto distinct exit codes here.

The ``status`` collision, resolved by a rule
--------------------------------------------
``hydromate status <config>`` has always meant *case* status. The job verb wants the same
...
_CODE_TO_EXIT = {
    "hydromate.config": EXIT_CONFIG,
    "hydromate.geodata": EXIT_GEODATA,
    "hydromate.environment": EXIT_ENVIRONMENT,
    "hydromate.solver": EXIT_SOLVER,
    "hydromate.mesh": EXIT_MESH,
}
...
def emit(command: str, data: Any, *, as_json: bool, lines: list[str] | None = None
    ...
    if as_json:
        ...
        print(json.dumps({"ok": True, "command": command, "hydromate": __version__,
...
def fail(command: str, exc: BaseException, *, as_json: bool, verbose: bool = False
    ...
    if as_json:
        print(json.dumps({"ok": False, "command": command, "hydromate": __version__,
...
def _setup_logging(args) -> None:
    ...
    for handler in logging.getLogger("hydromate").handlers:
...
def _submit_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--kind", help="what to run; see 'hydromate submit --help-kinds'")
...
def run_submit(argv: list[str]) -> int:
    ...
    _setup_logging(args)
    from hydromate.jobs.model import KIND_META
    ...
    try:
        ...
        from hydromate.jobs.profiles import resolve as resolve_profile
        ...
        if not args.kind:
            ...
            raise ConfigError(
                ...
                subject="kind",
                remedy="Run 'hydromate submit --help-kinds' to see them.",
            )
...
def _submit_existing_spec(args) -> int:
    """``hydromate submit job.json`` - the plan's literal form."""
    ...
    from hydromate.jobs import executor
...
def run_cancel(argv: list[str]) -> int:
    ...
    _setup_logging(args)
    try:
        from hydromate.jobs.launcher import LaunchHandle, launcher_for
        from hydromate.jobs.model import JobState
        from hydromate.jobs.reaper import reconcile
        from hydromate.jobs.store import read_status
        ...
        jd.cancel_request.write_text(
            json.dumps({"requested": _now(), "by": "hydromate cancel"}) + "\n",
            encoding="utf-8")
...
def _logs_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="hydromate logs",
        description="Show or locate a job's log. Two streams: hydromate's own "
...
def run_list(argv: list[str]) -> int:
    ...
    _setup_logging(args)
    try:
        from hydromate.jobs.index import list_jobs
...
def run_profiles(argv: list[str]) -> int:
    ...
    _setup_logging(args)
    try:
        from hydromate.jobs.paths import profiles_path
...
def _resolve_job(target: str | Path, *, job_root: Path | None = None):
    ...
    from hydromate.core.errors import ConfigError
    from hydromate.jobs import ids, paths
    from hydromate.jobs.paths import JobDir
    ...
    if ids.is_job_id(text):
        ...
        raise ConfigError(
            ...
            subject=text,
            remedy="Run 'hydromate list' to see the jobs that exist, or pass "
