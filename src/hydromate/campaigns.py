...
Three source layouts are handled (``kind`` on :class:`~hydromate.config`-level
...
* ``adapter`` - a revised summary workbook whose sheet already has one row per
  ...
  :func:`hydromate.ground_truth.read_flowtracker`); the standard hydromate path.
...
from hydromate.ground_truth import read_flowtracker, read_xlsx_sheet
