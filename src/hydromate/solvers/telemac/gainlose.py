...
(:mod:`hydromate.solvers.telemac.watertable`) each node's role follows from a head comparison:
...
(:mod:`hydromate.solvers.telemac.fortran`) applies the same two rules at run time, so the faces move
...
def derive_faces(cfg, mesh, plane, surface, *, tol: float = HEAD_TOLERANCE
    ...
    *surface* is a per-node free-surface elevation (bed + depth). A node is assigned
    ...
    from hydromate.solvers.telemac.watertable import water_table_depth
    ...
    from hydromate.solvers.telemac.wetting import _nodal_areas
