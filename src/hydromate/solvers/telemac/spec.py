...
Import-light by contract (see :mod:`hydromate.solvers`): standard library, plus
attribute access on a ``Config``. Nothing here imports ``steering``, ``selafin``,
gmsh or TELEMAC's Python, because ``hydromate status`` and a future QGIS plugin read
...
# the three 3D steering files add3d.py writes (hydromate.threed)
...
SPEC = BackendSpec(
    ...
    config_key="telemac",
    implementation="hydromate.solvers.telemac.backend:BACKEND",
