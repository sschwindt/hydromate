...
:mod:`hydromate.solvers.telemac.fortran`). The bar's saturated zone is therefore bounded by two known
...
* the **pre-wet** (:func:`hydromate.steering.write_initial_conditions`) seeds any bar
...
def _internal_lines(cfg):
    ...
    from hydromate.solvers.telemac.boundary import _is_internal, _type_column
...
def _samples_from_lines(cfg, mesh, surface, overrides):
    ...
    from hydromate.solvers.telemac.boundary import _internal_sign
...
def _samples_from_zone(cfg, mesh, surface, overrides, *, band: float = 10.0):
    ...
    from hydromate.solvers.telemac.steering import _centerline_arclength
...
def patch_node_mask(cfg, mesh):
    ...
    from hydromate.solvers.telemac.boundary import _percolation_patches
