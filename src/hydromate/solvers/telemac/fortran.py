...
def patch_drain_regions(cfg: Config, used: list) -> list:
    ...
    already covers it.
    """
    from hydromate.solvers.telemac.boundary import (
...
def write_user_fortran(cfg: Config, regions: list, *, plane=None) -> Path:
    ...
    *regions* is the :func:`hydromate.boundary.load_internal_source_regions` list
    (signed discharges; in fortran mode the losing region polygon is the
    percolation patch). *plane* is an optional
    :class:`hydromate.watertable.PhreaticPlane`: when given, the patch drain tapers
    ...
    """
    from hydromate.solvers.telemac.steering import _region_coords
    ...
    comment("hydromate-generated USER_RAIN: percolation through a porous patch")
...
def write_water_table_fortran(cfg: Config, plane) -> Path:
    ...
    * table above the bed -> **gaining** (the saturated zone cuts the ground).
    ...
    """
    from hydromate.solvers.telemac.boundary import _percolation_patches, _simplify_region
    ...
    comment("hydromate-generated USER_RAIN: gain-lose reach, faces from the")
