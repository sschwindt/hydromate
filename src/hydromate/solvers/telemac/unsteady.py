...
* **Boundaries are NOT re-defined.** hydromate's generated ``boundaries.cli`` is
...
``liquid-boundaries.json`` the build wrote (:func:`hydromate.boundary.dump_liquid_boundaries`),
...
from hydromate.solvers.telemac import boundary, steering
...
def write_control_sections(cfg: Config) -> Path | None:
    ...
    counts: dict[str, int] = {}
    lines = ["# control sections generated by hydromate (unsteady flux verification)",
             f"{len(runs)} -1"]
