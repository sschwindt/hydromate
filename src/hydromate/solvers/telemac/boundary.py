...
from hydromate.core.boundaries import (  # noqa: F401
    _flow_column, _is_internal, _normalise_kind, _type_column,
    liquid_line_details, liquid_lines,
)
from hydromate.solvers.telemac.mesh import Mesh, _anisotropic_enabled
...
def dump_liquid_boundaries(liquids: list["LiquidBoundary"], path: str | Path) -> Path:
    ...
    Written during the build (:mod:`hydromate.solvers.telemac.pipeline`) so the standalone unsteady /
