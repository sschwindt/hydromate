...
# Historic module names stay listed: they are shims (see hydromate/mesh.py and
# friends) that alias the moved modules, so `from hydromate import pipeline`
...
__all__ = [
    ...
    "Config", "FlowSpec", "FluxConvergence", "MeshValidity", "OutletProfile",
    "PhreaticPlane", "ProgressBar", "SolverBackend", "SolverProgress",
    "SolverStatus", "Sortie", "Support", "WettingReport",
...
if TYPE_CHECKING:  # give type checkers and IDEs the real names
    ...
    from hydromate.solvers.telemac.flux_convergence import (
    ...
    from hydromate.solvers.telemac.mesh import (
        build_mesh, channel_node_mask, interpolate_elevations, interpolate_roughness,
        write_mesh,
    )
    from hydromate.solvers.telemac.mesh_quality import assess_quality
    ...
    from hydromate.solvers.telemac.sections import line_discharges, write_line_discharges
    from hydromate.solvers.telemac.sortie import (
        find_lines, latest_sortie, read_sortie, sediment_mass_profile, Sortie,
        tracer_mass_profile,
    )
    from hydromate.solvers.telemac.steering import eddy_viscosity_estimate, select_turbulence_model
    from hydromate.targets import (
        read_target_parameters, read_targets, write_target_template,
    )
    from hydromate.solvers.telemac.threed import (
        build_3d_cas, build_3d_cases, infer_vertical_layers, select_3d_turbulence,
    )
    from hydromate.solvers.telemac.unsteady import (
    ...
    from hydromate.solvers.telemac.watertable import (
        fit_phreatic_plane, patch_node_mask, PhreaticPlane, water_table_depth,
    )
    from hydromate.solvers.telemac.wetting import (
