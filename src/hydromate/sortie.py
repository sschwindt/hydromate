...
**Why hydromate parses this itself.** The obvious alternative is TELEMAC's own
``postel.parser_output`` (and the ``pythomac`` package that adapts it), but both are
GPL-3 while hydromate is BSD-3, so neither can be vendored here. The file *format*
...
one, because hydromate needs exactly one thing from the listing:
...
# whose flux/convergence post-processing hydromate used to call out to. The field
