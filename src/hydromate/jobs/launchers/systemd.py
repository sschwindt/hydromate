...
``--collect``
    The unit name is deterministic (``hydromate-<job_id>.service``), which is what lets a
...
from hydromate.jobs.launcher import LaunchHandle, LaunchState, write_env_file

log = logging.getLogger("hydromate.jobs.launchers.systemd")

UNIT_PREFIX = "hydromate-"
...
class SystemdUserLauncher:
    ...
    def submit(self, job_dir: Any, argv: list[str], *, env: dict[str, str],
        ...
        command = [
            ...
            f"--description=hydromate job {job_id}",
