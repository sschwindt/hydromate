...
idling the VM out will terminate running jobs regardless of anything hydromate does. Such
...
from hydromate.core.environment import EnvironmentKind
...
log = logging.getLogger("hydromate.jobs.launchers.wsl")
...
class WslLauncher:
    ...
    def submit(self, job_dir: Any, argv: list[str], *, env: dict[str, str],
        ...
        distro = self.distro or env.get("HYDROMATE_WSL_DISTRO") or None
        # Every path crosses the boundary, so every path is translated - once, here.
        from hydromate.core.environment import windows_to_wsl
