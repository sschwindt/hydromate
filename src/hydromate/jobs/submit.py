...
from hydromate.core.environment import ENTERED_MARKER
from hydromate.jobs import ids, index, launcher as launcher_mod, paths, profiles
...
log = logging.getLogger("hydromate.jobs.submit")
...
def build_spec(cfg: Any, kind: str | JobKind, *, profile: profiles.Profile | None = None,
    ...
    from hydromate.config import config_to_dict
    ...
    return JobSpec(
        ...
        created_by={
            ...
            "hydromate": _version(), "argv": list(sys.argv[1:]),
        },
        case={
...
def _validate_and_capture(profile: profiles.Profile, env, spec: JobSpec
    ...
    if not status.ok:
        raise errors.EnvironmentError(
            ...
            remedy="Fix the setup script, then run 'hydromate profiles validate'.",
...
def _launch(jd: JobDir, spec: JobSpec, env, captured: dict[str, str],
    ...
    captured["HYDROMATE_JOB_DIR"] = str(jd.root)
    captured["PYTHONUNBUFFERED"] = "1"
    if env.distro:
        captured["HYDROMATE_WSL_DISTRO"] = str(env.distro)
