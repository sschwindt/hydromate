...
``mkdir(exist_ok=False)`` (see :mod:`hydromate.jobs.paths`), not asserted by entropy, so
...
#: ``hydromate status <arg>`` tell a job id from a case-config path without guessing.
    ...
    if match is None:
        raise ConfigError(
            ...
            remedy="Job ids look like 2026-08-14-isar-2025-steady-a3f19c; "
                   "run 'hydromate list' to see the known ones.",
        )
    return (
