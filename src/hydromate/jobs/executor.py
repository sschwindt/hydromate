...
This is the only module in :mod:`hydromate.jobs` that touches a solver, and it touches
...
Every failure becomes a :class:`~hydromate.core.errors.ErrorRecord` in ``status.json``
...
from hydromate.core import errors, registry
from hydromate.core.capabilities import Capability
from hydromate.core.errors import ErrorRecord
from hydromate.jobs import events, logs, results
from hydromate.jobs.interaction import PolicyAsk
from hydromate.jobs.lock import JobLock
from hydromate.jobs.model import (KIND_META, JobKind, JobSpec, JobState, JobStatus,
                                  utc_now)
from hydromate.jobs.paths import JobDir
from hydromate.jobs.store import read_spec, read_status, write_status

log = logging.getLogger("hydromate.jobs.executor")
...
def rebase(cfg: Any, job_dir: JobDir) -> None:
    ...
    Four assignments and nothing else, because every path in hydromate already resolves
    ...
    if mode == "link":
        ...
        if not source.exists():
            raise errors.ConfigError(
                ...
                remedy="Run 'hydromate list' to see the jobs that exist.",
            )
...
def execute(job_dir: JobDir | str | os.PathLike, *, sink: Any = None,
    ...
    Synchronous by design - this is what ``hydromate execute`` calls, and what every
...
def _load_backend(spec: JobSpec):
    try:
        ...
        raise errors.EnvironmentError(
            ...
            subject="solver",
            remedy="Reinstall hydromate, or check the plugin providing this backend.",
            cause=str(exc),
        ) from exc
...
def _check_environment(backend: Any, cfg: Any, spec: JobSpec) -> None:
    ...
    try:
        ...
        raise errors.EnvironmentError(
            ...
            remedy="Run 'hydromate profiles validate' to see what is missing.",
            cause=str(exc),
        ) from exc
    if ok is False:
        raise errors.EnvironmentError(
...
_FAILURE_HINTS: tuple[tuple[type[BaseException], str, str], ...] = (
    (FileNotFoundError, "hydromate.config",
     "A file the job needs is missing. Check the paths in the frozen "
     "input/case-config.yml."),
    (PermissionError, "hydromate.environment",
     "hydromate could not read or write a path the job needs."),
    (MemoryError, "hydromate.environment",
...
def _classify(exc: BaseException, spec: JobSpec) -> ErrorRecord:
    if isinstance(exc, errors.HydromateError):
