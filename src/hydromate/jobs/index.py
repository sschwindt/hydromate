...
from hydromate.jobs.ids import JOB_ID_RE
from hydromate.jobs.paths import JobDir

log = logging.getLogger("hydromate.jobs.index")
...
def row_for(job_dir: JobDir | str | os.PathLike, *, reconcile: bool = False
    ...
    try:
        from hydromate.jobs.store import read_spec, read_status
        spec = read_spec(jd)
    ...
    try:
        if reconcile:
            from hydromate.jobs.reaper import reconcile as _reconcile
