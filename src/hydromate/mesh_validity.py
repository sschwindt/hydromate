...
* the **turbulence-model consistency**: what
  :func:`hydromate.steering.turbulence_pick_for_dx` *would* auto-select at this
...
def channel_ks(cfg: Config) -> float | None:
    ...
    Resolution order mirrors :func:`hydromate.steering._friction_rows`:
...
def _turb_name(model: int | None) -> str:
    from hydromate.steering import TURB_NAMES
