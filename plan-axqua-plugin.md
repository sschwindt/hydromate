# Task: Turn `axqua` into a QGIS-integrated, persistent TELEMAC/OpenFOAM optimization system

The objective is to integrate aXqua as a backend into a QGIS plugin as a frontend and a robust external job execution architecture around `axqua`.

`axqua` repository is now public.

**Licence: no change needed.** `axqua` is BSD-3-Clause, which is GPL-compatible - it is permissive and carries no advertising clause (unlike the old 4-clause BSD), so a GPLv2+ QGIS plugin may incorporate and link it. The plugin itself must be GPLv2+ because it links PyQGIS; `axqua` stays BSD-3 and remains usable outside the plugin. Confirm with counsel if the project needs a formal opinion, but the expected action is none. Because no OpenFOAM or TELEMAC source is redistributed and this is pre-/post-processing only, licensing is independent of those.
...
project targets (openfoam.org - the build aXqua's dictionary writer is verified
...
Plugin setups are defined in `<project-name.axqua-prj` files that contain all user settings and simulation status.
...
```text
axqua + TELEMAC/OpenFOAM shall keep running.
```

When QGIS is started again, the plugin must rediscover those jobs and display their current status.

The QGIS plugin is therefore a **frontend/job-control client**, while `axqua` remains the external execution engine.

---

# 2. What already exists in aXqua (do not rebuild these)

This section answers, concretely, the "identify what exists" step. It is current as of
the solver-backend split (`axqua.core` / `axqua.solvers`).

| the plan needs | already in aXqua | notes |
|---|---|---|
| solver abstraction (§12) | `core/registry.py`: `BackendSpec` (metadata) + `SolverBackend` (protocol) | **Do not introduce a second `SolverAdapter`.** Extend this one. Backends are discovered through the `axqua.solvers` entry-point group, so a new solver is a plug-in, not a fork. |
...
| structured failures | **`core/errors.py`**: `AxquaError` + 5 categories, each with a stable `code`, optional `subject` (the config key at fault) and `remedy`; `ErrorRecord.from_exception` | **Done (phase 4d).** §5's `status.json` should store `ErrorRecord.as_dict()`; the plugin should show `remedy` and highlight `subject`. |
...
| CLI | `axqua <config>`, `status`, `openfoam`, `clip`, `rating`, `targets`, **`migrate`** | Missing the job verbs: `submit`, `execute`, `cancel`, `logs`, `list`. |
...
* **the plugin must never `import axqua`** - QGIS's Python is not the solver's
  Python. The plugin talks to aXqua over the CLI and over files, only.
...
| phase | what | where |
|---|---|---|
| 1 | capabilities, solver registry, `MODEL=` markers, `axqua status` | `core/capabilities.py`, `core/registry.py`, `solvers/*/spec.py` |
...
| 4c | `dump_config`, `axqua migrate` | `config.py`, `cli.py` |
...
| **7-11** | **the QGIS plugin**: compat layer, runner client, `.axqua-prj`, capability-generated tabs, job dashboard, result loading, Processing provider, A3 layout | **`qgis_plugin/axqua/`** |
...
* **`import axqua` must stay light.** `tests/test_capabilities.py` asserts from a
...
The plan names four artifacts - `case-config.yml`, `<project>.axqua-prj`,
...
| `<project>.axqua-prj` | a **thin project pointer**: which case configs belong to this project, which solver profile to use, where the job root is, plugin view state | the plugin only | user-owned, git-friendly |
| `~/.config/axqua/profiles.yml` | **machine-local** solver profiles: setup scripts, MPI counts, scratch roots | user / plugin settings dialog | never in the project file - these are machine-specific paths and would destroy portability |
...
**`.axqua-prj` must not contain simulation status.** The original draft has it hold
...
```bash
axqua submit job.json
axqua execute job.json
axqua status JOB_ID
axqua cancel JOB_ID
axqua logs JOB_ID
axqua list
...
Revise the `axqua` directory structure robustly so that each case approximately write like:
...
```text
~/.local/share/axqua/jobs.sqlite
```

but it must be reconstructible by scanning the job root, and `axqua list --rebuild`
...
Do not assume this exact schema if the existing `axqua` configuration model is already suitable.
...
One implementation owns all of this (`axqua.core.environment.SolverEnvironment`),
...
Each aXqua job should have a deterministic unit name such as:

```text
axqua-<job-id>.service
```
...
* **the distro's lifetime is not ours to control.** `wsl --shutdown`, a Windows restart,
  or WSL2 idling the VM out will terminate running jobs regardless of what aXqua
...
**It already exists - extend it, do not add a second one.** `axqua.core.registry`
splits the abstraction in two, deliberately:

```python
BackendSpec      # metadata only: name, capability support matrix, artifact predicates.
                 # Import-light by contract, so `axqua status` and the plugin can
...
Backends register through the `axqua.solvers` entry-point group, so a third solver
is `pip install axqua-basement`, not a fork. `axqua/solvers/telemac/spec.py` and
`axqua/solvers/openfoam/spec.py` are the worked examples.
...
`axqua.core` imports a solver, and one backend never reaches into another. Both are
...
Solver support according to existing aXqua setups

## TELEMAC support

Reuse existing TELEMAC support in `axqua`.
...
Create the plugin as a separate package/module around `axqua`.
...
**Generate the tab set from aXqua's capability matrix rather than hardcoding it.**
`axqua status <config> --full` already reports, per solver, every capability on
...
* **adding a solver or a capability to aXqua surfaces in the plugin with no plugin
  change**, which is the whole point of the registry.

The plugin obtains this as JSON from the CLI. It must never import aXqua to ask.
...
Alternatively, users can select `<project-name.axqua-prj` files that contain all user settings.


## Preprocessing, Steady jobs, Mesh convergence, BAL, Unsteady jobs

Adapt from aXqua workflow.
...
# 19. Rediscover jobs when a `.axqua-prj` is loaded

On plugin initialization:

1. find the configured aXqua job registry/root;
...
5. User opens plugin and selects the `.axqua-prj` file: Plugin discovers the existing job.
...
Adapt `axqua.logsetup` (per-phase compound logfile, `log_step()` timings).

**Add rotation, which aXqua does not currently have.** A multi-day parallel run
...
aXqua's narration (state transitions, commands issued, exit codes); the solver's
...
Where `axqua` itself requires installation, implement a clear discovery mechanism.

For example:

```text
configured axqua executable
```
...
Validate the find by running `axqua --version` and checking it parses. A wrong or
...
QGIS may use one Python installation while TELEMAC/axqua uses another.

The plugin should invoke `axqua` as an external executable/process rather than importing solver-specific environments into QGIS.

---

# 29. Preserve standalone aXqua usage
...
Adopt the existing aXqua developer and user documentation.
...
`axqua execute` working without QGIS.
...
6. Confirm TELEMAC and axqua continue running.
7. Reopen QGIS.
...
## E. QGIS restart and `.axqua-prj` file loading
