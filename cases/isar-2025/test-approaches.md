...
`hydromate-case/simulation/steady2d.cas_2026-07-27-09h33min26s/`:
...
dictionary - also governs the **Spalart-Allmaras** solve. hydromate now raises it
...
(Areas are the values hydromate reports at build time; the patch region is ~24x
...
Generated by `hydromate/fortran.py` into `<model_dir>/user_fortran/user_rain.f` and
...
`RAIN OR EVAPORATION : YES` with a base rate of 0 (hydromate writes both).
...
| tree | `hydromate-case/scenarios/prescribed-q/` | `hydromate-case/scenarios/green-ampt/` |
...
Each rung builds into its **own** folder (`hydromate-case/ladder-<rung>/`), so
nothing overwrites the production `simulation/` case and rungs can be compared
side by side:

```bash
cd /srv/private/hydromate
export PYTHONPATH=src        # hydromate is not pip-installed in hydromate-env here

# one rung, build + run (add --ncsize N to change the core count)
mamba run -n hydromate-env python cases/isar-2025/ladder.py T2

# build only, e.g. to inspect the generated .cas / source-regions.txt first
mamba run -n hydromate-env python cases/isar-2025/ladder.py T3 --build-only
```
...
**Note on `pythomac`:** hydromate's `analyze_flux_convergence` (and therefore
...
**Also note:** `hydromate-env` has no `pytest`/`ruff`, so the test suite was run
from the base conda environment (which has every runtime dependency). The solver
runs use `hydromate-env` as documented.
...
case (`hydromate-case/simulation/`) - the ladder folders are scratch:
...
3. rebuild and run:
   ...
   ```bash
   mamba run -n hydromate-env python cases/isar-2025/preprocessing.py
   mamba run -n hydromate-env python cases/isar-2025/initial_run.py
   ```
...
`hydromate/steering.py::write_source_regions` and is covered by tests.
...
**The fix** (`hydromate.steering.spill_elevations`, new
...
(`hydromate.rating.section_rating` /`synthesize_outflow_rating_from_section`): the
...
New module `hydromate.sections`, wired into `initial_run.py`, integrates
...
`pythomac` is still not importable in `hydromate-env`, so the four
...
### R2.6 Measuring it: `hydromate/wetting.py`
...
a plane (`hydromate/watertable.py`):
...
## Revision R4 - 2026-08-07 - the gain-lose machinery becomes core hydromate
...
* **Disk:** `hydromate-case/simulation/` currently holds ~50 GB, most of it the
