...
Example:
    python plot_telemac_discharge_convergence.py t3d_case.sortie --qtar 2.0

Example with CSV export:
    python plot_telemac_discharge_convergence.py t3d_case.sortie --qtar 2.0 --csv convergence.csv

Author: generated for TELEMAC .sortie discharge-convergence checking.
"""
...
def plot_convergence(
    ...
    """Create the discharge convergence plot."""
    ...
    listing_dt = infer_listing_dt(times)

    fig, ax = plt.subplots(figsize=(10.5, 5.8))

    ax.plot(times, q_in, color="red", linewidth=1.6, label="Inflow |Q|")
    ax.plot(times, q_out, color="blue", linewidth=1.6, label="Outflow |Q|")
    ax.axhline(qtar, color="purple", linestyle="--", linewidth=1.4, label=f"Qtar = {qtar:g} m³/s")
    ...
    ax.set_ylim(0.5 * qtar, 1.5 * qtar)

    ax.set_xlabel("Listing printout time [s]")
    ax.set_ylabel("Boundary discharge magnitude |Q| [m³/s]")
    ...
    # Grid definition requested by user.
    # x major grid = 10 * listing printout interval.
    if listing_dt is not None and listing_dt > 0.0:
        ax.xaxis.set_major_locator(MultipleLocator(10.0 * listing_dt))
        ax.xaxis.set_minor_locator(MultipleLocator(listing_dt))

    # y-grid-major = 1/10 of discharge fluctuation between 0.5Qtar and 1.5Qtar.
    # The fluctuation width is Qtar, so major = Qtar/10, minor = Qtar/20.
    if qtar > 0.0:
        ax.yaxis.set_major_locator(MultipleLocator(qtar / 10.0))
        ax.yaxis.set_minor_locator(MultipleLocator(qtar / 20.0))
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            ...
            "convergence against a target Q."
        )
    )
    p.add_argument(
        "sortie",
    ...
    p.add_argument(
        ...
        "--output",
        type=Path,
        default=Path("discharge_convergence.png"),
        help="Output plot file. Default: discharge_convergence.png",
    )
    p.add_argument(
        "--csv",
        type=Path,
...
path = Path("/mnt/data/plot_telemac_discharge_convergence.py")
