"""Mesh-convergence (grid-independence) study for the Inn case (workflow step 2).
...
creates and works in its own ``tm-simulation/mesh-convergence/`` folder: a styled
...
Each mesh is **pre-wetted**: the channel is seeded with ``INITIAL_DEPTH`` (0.5 m
by default) of water on the nodes inside the ``channel`` mesh-zones, and the run
is continued from that warm start, so the solver need not advance the wetting
front from a dry bed (a large time saving across the five runs). Set
``INITIAL_DEPTH = None`` to start from a dry bed instead.
...
def prepare_inputs(cfg, q: float, initial_depth: float | None = INITIAL_DEPTH) -> None:
    """Prescribe the discharge and synthesise the inflow + outflow rating if missing
    (so every mesh in the study builds from the same boundary conditions), and
    pre-wet the channel to *initial_depth* so each run warm-starts from a wet
    channel instead of advancing the wetting front from a dry bed."""
    cfg.hydrodynamics.prescribed_flowrate = q
    cfg.hydrodynamics.prewet_depth = initial_depth
...
def main() -> None:
    cfg.ensure_dirs()
    # the study creates and works in its own tm-simulation/mesh-convergence/ folder
