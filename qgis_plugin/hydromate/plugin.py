...
class HydromatePlugin:
    ...
    def initGui(self) -> None:      # noqa: N802 - QGIS calls this name
        from .gui.dock import HydromateDock

        self.dock = HydromateDock(self.iface, self.iface.mainWindow())
        self.iface.addDockWidget(DOCK_RIGHT, self.dock)
        self.dock.hide()

        show = QAction(icon("hydromate.svg"), "HydroMate panel",
    ...
    def _add_provider(self) -> None:
        try:
            ...
            from .processing.provider import HydromateProvider
            self.provider = HydromateProvider()
            ...
            self.iface.messageBar().pushMessage(
                "HydroMate", f"Processing algorithms unavailable: {exc}", duration=5)
    ...
    def _add_layout(self) -> None:
        ...
        try:
            ...
            self.iface.messageBar().pushMessage(
                "HydroMate", f"Could not create the layout: {exc}", duration=8)
            return
        self.iface.messageBar().pushMessage(
            "HydroMate", f"Added the print layout {name!r}. Open it from "
