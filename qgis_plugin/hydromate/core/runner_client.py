"""Talking to hydromate - over the command line, and only over the command line.

**The plugin never imports hydromate.** That is not a stylistic preference; it is what
makes the whole thing work. QGIS ships its own Python, and hydromate needs gmsh,
...
to something that might be a different hydromate:
...
The find is validated by running ``hydromate --version`` **when it is configured**, not
...
ENV_VAR = "HYDROMATE_EXE"
SETTINGS_KEY = "hydromate/executable"
...
class RunnerError(RuntimeError):
    """A hydromate call failed. Carries the structured error when there was one."""
...
class RunnerNotFound(RunnerError):
    """hydromate could not be located. The message names every place looked."""
...
class RunnerInfo:
    ...
    def describe(self) -> str:
        return f"hydromate {self.version} at {self.path} (found via {self.source})"
...
def find_executable(configured: str | None = None) -> RunnerInfo:
    """Locate and validate hydromate, or raise with an actionable message."""
    ...
    on_path = shutil.which("hydromate")
    if on_path:
    ...
    lines = ["hydromate could not be found.", "", "Looked in:",
    ...
    lines += ["", "Install it with 'pip install hydromate' in the environment that has "
              "your solver tooling, then set the path in HydroMate > Settings."]
...
class RunnerClient:
    """A thin, synchronous wrapper around the hydromate CLI.

    Synchronous on purpose: every call is short. Callers that must not block the GUI
    wrap *this* in a ``QgsTask`` (see :mod:`hydromate_plugin.core.tasks`), which is the
    ...
    def call(self, args: Sequence[str], *, timeout: float | None = None,
        ...
        try:
            ...
            raise RunnerError(
                f"hydromate did not answer within {timeout or self.timeout:.0f}s",
                remedy="The job root may be on a slow or unreachable filesystem.",
            ) from exc
        except (OSError, subprocess.SubprocessError) as exc:
            raise RunnerError(f"could not run hydromate: {exc}") from exc
        ...
        if payload is None:
            raise RunnerError(
                "hydromate returned something that is not JSON",
        ...
        if not result.ok:
            error = result.error
            raise RunnerError(str(error.get("message") or "hydromate reported a failure"),
...
def _parse(text: str) -> dict | None:
    ...
    hydromate sends narration to stderr precisely so this is unnecessary - but a stray
