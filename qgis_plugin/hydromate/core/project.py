"""``<name>.hydromate-prj``: a thin pointer, deliberately.
...
* ``<name>.hydromate-prj`` - which cases belong to this project, which solver profile to
...
SUFFIX = ".hydromate-prj"
SCHEMA_VERSION = 1


@dataclass
class HydromateProject:
    ...
    def from_dict(cls, data: dict[str, Any], *, path: Path | None = None
        ...
        if version > SCHEMA_VERSION:
            raise ValueError(
                f"this project file was written by a newer HydroMate "
...
def load(path: str | os.PathLike) -> HydromateProject:
    target = Path(path)
    payload = json.loads(target.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError(f"{target} is not a HydroMate project file")
    return HydromateProject.from_dict(payload, path=target)


def save(project: HydromateProject, path: str | os.PathLike | None = None) -> Path:
