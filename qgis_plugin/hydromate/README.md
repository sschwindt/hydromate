...
the separately installed [`hydromate`](https://github.com/sschwindt/hydromate)
command-line tool, which owns the model build, the solver runs and the calibration.

That separation is the whole design. QGIS ships its own Python, while hydromate needs
...
the boundary is a subprocess and a JSON document — **the plugin never imports hydromate**.
...
distro under WSL). Reopen QGIS days later, open your `.hydromate-prj`, and the jobs are
...
| hydromate | `pip install hydromate` (version 0.2 or newer), in an environment that can reach your solver |
...
1. Install hydromate in the environment that already has your solver tooling, and check
   it works: `hydromate --version`.
2. In QGIS, install this plugin, then open **HydroMate** from the toolbar.
3. On the **Setup** tab, press **Check**. If hydromate is not on `PATH`, set its path in
...
4. **Add case…** and pick a `case-config.yml`, then **Save as…** to write a
   `.hydromate-prj` next to it.

Optionally describe your solver installs once, in `~/.config/hydromate/profiles.yml`, and
select the profile on the Setup tab. Run `hydromate profiles validate` to check them.

## Using it

The tabs between **Setup** and **Jobs** are **generated from what hydromate says your case
can do**, not hardcoded. A capability that does not apply to your solver is hidden; one
hydromate has not implemented yet is shown disabled with the reason. Add a capability to
hydromate and it appears here with no plugin update.
...
Results arrive as styled layers under `hydromate/<job id>`: water depth on a white-to-blue
...
|---|---|---|
| the model | `case-config.yml` | you (or hydromate) |
| which cases and where jobs go | `<name>.hydromate-prj` | this plugin |
| your solver installs | `~/.config/hydromate/profiles.yml` | you |
| what a job was asked to do | `<job>/job.json` | hydromate, once |
| what a job is doing now | `<job>/status.json` | hydromate's runner |
...
**"hydromate could not be found"** — the message lists all three places it looked (the
plugin setting, `$HYDROMATE_EXE`, `PATH`). Set the path in Settings and press Test.

**A job shows FAILED immediately** — open **View logs**. `runner.log` is hydromate's
narration and carries the error with a suggested remedy; the solver's own listing is
behind the toggle.

**A job is stuck in RUNNING** — it will not be. hydromate reconciles a recorded process
...
GPL-2.0-or-later, because it links PyQGIS. The `hydromate` package it drives is
