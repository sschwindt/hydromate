...
The tab set is rebuilt whenever the case changes, from ``hydromate case-status --json``.
That is the design point (plan §17): the plugin does not know what TELEMAC or OpenFOAM can
do, it *asks*, so a capability added to hydromate appears here without a plugin release.
...
class PluginContext:
    ...
    def __init__(self, iface, dock) -> None:
        ...
        self.project = project_io.HydromateProject()
        self.case_view: CaseView | None = None
        self.runner_ok = False

    # -- messaging ----------------------------------------------------------------
    def _message(self, text: str, level) -> None:
        self.iface.messageBar().pushMessage("HydroMate", text, level=level, duration=8)
    ...
    def client_or_warn(self) -> RunnerClient | None:
        ...
        Every action goes through this, so "hydromate is not installed" is reported once,
...
class HydromateDock(QDockWidget):
    """The plugin's main window."""

    def __init__(self, iface, parent=None) -> None:
        super().__init__("HydroMate", parent)
        self.iface = iface
        self.setObjectName("HydromateDock")
        ...
        self._placeholder = QLabel(
            ...
            "what hydromate reports this case can actually do.")
    ...
    def refresh_case(self) -> None:
        ...
        run_async("HydroMate: reading what this case can do",
    ...
    def _rebuild_tabs(self, view: CaseView | None) -> None:
        ...
        for solver in view.enabled_solvers:
            for capability in solver.visible_capabilities:
                ...
                # Shown but disabled where hydromate has not implemented it - a gap in
                # hydromate is worth seeing, and is different from a category error,
