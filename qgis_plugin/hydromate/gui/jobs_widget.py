...
class JobsWidget(QWidget):
    ...
    def refresh(self) -> None:
        """Ask hydromate for the job list, off the GUI thread."""
        client = self.ctx.client_or_warn()
        if client is None:
            return
        job_root = self.ctx.project.job_root or None
        case = None
        run_async("HydroMate: listing jobs",
    ...
    def _cancel(self) -> None:
        ...
        if client is None:
            return
        job_root = self.ctx.project.job_root or None
        run_async(f"HydroMate: cancelling {job.job_id}",
    ...
    def _load_results(self) -> None:
        ...
        self.ctx.info(f"Loaded {len(added)} layer(s) into hydromate/{job.job_id}.")
