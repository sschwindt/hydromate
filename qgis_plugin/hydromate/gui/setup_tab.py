...
Until hydromate can be found and a case is chosen, no other tab can do anything useful,
...
It is also where a ``.hydromate-prj`` is opened, which is the whole rediscovery story
...
class SetupTab(QWidget):
    ...
    def _build(self) -> None:
        ...
        # -- hydromate itself -----------------------------------------------------
        runner_box = QGroupBox("hydromate")
        ...
        self.job_root_edit.setPlaceholderText(
            "hydromate's default (a large scratch volume is a good idea)")
    ...
    def load_profiles(self) -> None:
        run_async("HydroMate: reading solver profiles",
    ...
    def open_project(self) -> None:
        path, _ = QFileDialog.getOpenFileName(
            self, "Open a HydroMate project", "",
            f"HydroMate project (*{project_io.SUFFIX})")
        if path:
    ...
    def save_project_as(self) -> None:
        path, _ = QFileDialog.getSaveFileName(
            self, "Save the HydroMate project", "",
            f"HydroMate project (*{project_io.SUFFIX})")
        if path:
    ...
    def add_case(self) -> None:
        path, _ = QFileDialog.getOpenFileName(
            self, "Add a case configuration", "", "hydromate case (*.yml *.yaml)")
        if path:
