"""Submit a hydromate job and return its id.
...
#: Offered in the enum. Kept short - the full list is in ``hydromate submit --help-kinds``
...
class SubmitJobAlgorithm(QgsProcessingAlgorithm):
    ...
    def shortHelpString(self) -> str:  # noqa: N802 - QGIS naming
        return (
            "Creates a persistent hydromate job and starts it detached, then returns "
            "the job id.\n\n"
            ...
            "HydroMate panel, or with the 'Check job status' algorithm.")
    ...
    def initAlgorithm(self, config=None):    # noqa: N802 - QGIS naming
        ...
        self.addParameter(QgsProcessingParameterFile(
            self.JOB_ROOT, "Job root (blank: hydromate's default)",
